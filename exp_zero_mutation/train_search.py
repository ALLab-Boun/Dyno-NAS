...
from evolution_enhanced import ZeroMutationController, mutate_zero
import copy
...
def main():
    ...
    zero_controller = ZeroMutationController(zero_mutation_prob=0.1, adaptive=True)
    ...
    while t < args.g_max:
        ...
        # Evolve population P by applying evolutionary operators with zero mutation.

        logging.info(f"Generation {t}: Evolving population with zero mutation...")
        ...
        # Calculate population mean fitness for adaptive zero mutation
        ...
        while len(new_population) < args.pop_size:
            ...
            if np.random.rand() < args.mutation_rate:
                was_skipped = mutate_zero(
                    child1, 
                    zero_controller,
                    parent_fitness=parent1.fitness,
                    population_mean_fitness=mean_fitness
                )
                child1._zero_mutation = was_skipped
                child1._parent_fitness = parent1.fitness if parent1.fitness is not None else 0.0
            ...
            if np.random.rand() < args.mutation_rate:
                ...
                child2._zero_mutation = was_skipped
                child2._parent_fitness = parent2.fitness if parent2.fitness is not None else 0.0
        ...
        # Update zero mutation controller with fitness feedback
        for individual in population:
            if hasattr(individual, '_zero_mutation') and individual.fitness is not None:
                zero_controller.update_statistics(
                    individual._zero_mutation,
                    individual.fitness
                )
        
        # Log zero mutation statistics periodically
        if t % 10 == 0:
            stats = zero_controller.get_statistics()
            logging.info(f"Zero Mutation Stats at Generation {t}: "
                        f"Zero count={stats['zero_mutation_count']}, "
                        f"Active count={stats['active_mutation_count']}, "
                        f"Zero mean fitness={stats['zero_mutation_mean_fitness']:.4f}, "
                        f"Active mean fitness={stats['active_mutation_mean_fitness']:.4f}")
        
        population.sort(key=lambda x: x.fitness if x.fitness is not None else -1, reverse=True)
        best_individual = population[0]
        logging.info(f"Best Fitness (Accuracy) for Generation {t}: {best_individual.fitness:.4f}")
