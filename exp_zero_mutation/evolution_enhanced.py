...
class ZeroMutationController:
    """
    Zero Mutation Controller - Strategic No-Mutation Operator
    
    Inspired by elitism and baseline preservation strategies in evolutionary algorithms.
    Sometimes the best mutation is no mutation at all.
    
    Key Features:
    - Selective preservation of individuals without mutation
    - Tracks effectiveness of zero mutation vs. active mutation
    - Useful for maintaining diversity and elite solutions
    - Provides control group for mutation effectiveness analysis
    """
    
    def __init__(self, zero_mutation_prob=0.1, adaptive=True):
        """
        Args:
            zero_mutation_prob: Base probability of applying zero mutation (no change)
            adaptive: Whether to adaptively adjust based on population fitness
        """
        ...
        # Statistics tracking
        self.zero_mutation_count = 0
        self.active_mutation_count = 0
        self.zero_mutation_fitness = []
        self.active_mutation_fitness = []
        
    def should_skip_mutation(self, individual_fitness=None, population_mean_fitness=None):
        """
        Determine whether to skip mutation for this individual
        
        Args:
            individual_fitness: Fitness of the individual being considered
            population_mean_fitness: Mean fitness of the population
        
        Returns:
            bool: True if mutation should be skipped
        """
        # Adaptive adjustment based on fitness
        if self.adaptive and individual_fitness is not None and population_mean_fitness is not None:
            # Higher fitness individuals are more likely to be preserved
            if individual_fitness > population_mean_fitness:
                adjusted_prob = min(self.base_prob * 1.5, 0.5)
            else:
                adjusted_prob = self.base_prob * 0.5
            
            self.current_prob = adjusted_prob
        else:
            self.current_prob = self.base_prob
        
        return np.random.rand() < self.current_prob
    
    def update_statistics(self, was_zero_mutation, fitness):
        """
        Update statistics for tracking mutation effectiveness
        
        Args:
            was_zero_mutation: Whether zero mutation was applied
            fitness: Resulting fitness of the individual
        """
        if was_zero_mutation:
            self.zero_mutation_count += 1
            if fitness is not None:
                self.zero_mutation_fitness.append(fitness)
        else:
            self.active_mutation_count += 1
            if fitness is not None:
                self.active_mutation_fitness.append(fitness)
    
    def get_statistics(self):
        """
        Get statistics about zero vs active mutation performance
        
        Returns:
            dict: Statistics including counts and mean fitness values
        """
        zero_mean = np.mean(self.zero_mutation_fitness) if self.zero_mutation_fitness else 0.0
        active_mean = np.mean(self.active_mutation_fitness) if self.active_mutation_fitness else 0.0
        
        return {
            'zero_mutation_count': self.zero_mutation_count,
            'active_mutation_count': self.active_mutation_count,
            'zero_mutation_mean_fitness': zero_mean,
            'active_mutation_mean_fitness': active_mean,
            'current_prob': self.current_prob
        }


def mutate_zero(individual, zero_controller, parent_fitness=None, population_mean_fitness=None):
    """
    Zero Mutation Operator
    
    Strategically skips mutation to preserve individuals. This is useful for:
    - Maintaining baseline comparisons
    - Preserving elite solutions
    - Reducing computational overhead
    - Studying mutation impact
    
    Unlike elitism (which preserves only the best), zero mutation can be applied
    to any individual based on adaptive criteria.
    
    Args:
        individual: Individual to potentially mutate
        zero_controller: ZeroMutationController instance
        parent_fitness: Fitness of the parent (optional)
        population_mean_fitness: Mean fitness of population (optional)
    
    Returns:
        was_skipped: True if mutation was skipped, False if standard mutation applied
    """
    # Determine if we should skip mutation
    should_skip = zero_controller.should_skip_mutation(
        individual_fitness=parent_fitness,
        population_mean_fitness=population_mean_fitness
    )
    
    if should_skip:
        # Zero mutation - do nothing, just mark individual for re-evaluation
        # Note: We don't reset fitness to None because the architecture hasn't changed
        # This preserves the inherited fitness from the parent
        return True
    else:
