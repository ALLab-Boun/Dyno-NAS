...
def main():
    ...
    # Initialize all mutation controllers for hybrid approach
    adaptive_controller = AdaptiveMutationController(num_operations=8, memory_size=10, target_size_ratio=1.2)
    ...
    while t < args.g_max:
        ...
        # Evolve population P by applying HYBRID mutation strategy.
        # Progress-based: Guided (early) -> Adaptive (middle) -> Soft (late)
        
        progress = t / args.g_max
        
        # Calculate diversity and adaptive mutation rate
        diversity = calculate_population_diversity(population)
        mutation_rate = periodic_controller.get_mutation_rate(diversity)
        
        logging.info(f"Generation {t}: Evolving population with HYBRID mutation (progress={progress:.2f}, mutation_rate={mutation_rate:.3f}, diversity={diversity:.3f})...")
        ...
        while len(new_population) < args.pop_size:
            ...
            # HYBRID MUTATION STRATEGY - phase-based approach
        ...
        # Update all mutation controllers with fitness feedback
        ...
        for individual in population:
            ...
            # Update adaptive mutation controller
            ...
            # Update soft mutation controller
            if hasattr(individual, '_soft_params'):
        ...
        # Update periodic controller's generation counter
        periodic_controller.update_generation()
        
        population.sort(key=lambda x: x.fitness if x.fitness is not None else -1, reverse=True)
        best_individual = population[0]
        
        # Enhanced logging with mutation type statistics
        mutation_types = {}
        for ind in population:
            if hasattr(ind, '_mutation_type'):
                mutation_types[ind._mutation_type] = mutation_types.get(ind._mutation_type, 0) + 1
        
        logging.info(f"Best Fitness (Accuracy) for Generation {t}: {best_individual.fitness:.4f}")
        logging.info(f"Mutation types used: {mutation_types}")
